...
class TransactionParser:
    ...
    def _compile_pattern(self) -> Optional[re.Pattern]:
        ...
        # Banks with custom parsers (Halifax, HSBC, Barclays) don't need patterns
        if self.config.bank_name.lower() in ['halifax', 'hsbc', 'barclays']:
            return None
    ...
    def parse_text(
        ...
        elif self.config.bank_name.lower() == 'barclays':
            logger.info("Routing to Barclays parser")
            return self._parse_barclays_text(text, statement_start_date, statement_end_date)
    ...
    def _parse_barclays_text(
        ...
        # Header pattern: "Date        Description                                          Money out           Money in            Balance"
        ...
        # Date pattern: "DD MMM" or "DD MMM YYYY" at start of line (max 5 spaces indent)
        ...
        transaction_start_pattern = re.compile(r'^\s{10,}(Card Payment|Direct Debit|Bill Payment|Received From|Standing Order|Cash machine|Automated Payment)', re.IGNORECASE)
    ...
    def _build_barclays_transaction(
        ...
        # Remove date from first line, clean up
